...
def plot_aligned():
    folder_2 = "/home/kia/Kiyanoush/UoLincoln/Projects/Tactile_control/tactile_prediction/uninversal_trainer/ACTP/saved_models/model_23_02_2022_10_38/test_results/test_trial_"

    for trial in range(539):
        ...
        plt.plot([i for i in range(len(ACTP_gt))], [None] + list(ACTP_gt[:-1, 0, 5]), c="r", label="Ground truth")

        # plt.plot([i for i in range(len(ACTP_pt))], ACTP_pt[:, 0, 5], label="CDNA t+0")
        plt.plot([i for i in range(len(ACTP_pt))], ACTP_pt[:, 3, 5], label="ACTP t+3")
        plt.plot([i for i in range(len(ACTP_pt))], ACTP_pt[:, 6, 5], label="ACTP t+6")
        plt.plot([i for i in range(len(ACTP_pt))], ACTP_pt[:, 9, 5], c="g", label="ACTP t+10")

        plt.grid(True)
        plt.title("ACTP Predictions - Trial: " + str(trial))
        plt.xlabel('time step')
        plt.ylabel('tactile reading')
        plt.legend(loc="upper right")
        # plt.show()
        plt.savefig('/home/kia/Kiyanoush/UoLincoln/Projects/Tactile_control/tactile_prediction/uninversal_trainer/ACTP/saved_models/model_23_02_2022_10_38/test_plots/trial_' + str(trial) + '_x.png')
        plt.close()
